...
class Auth extends CI_Controller
{
    ...
    public function registration()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            Congratulati0n!, your account has been created. Please Login</div>');
            redirect('auth');
        }
    }
    ...
    private function _sendEmail($token, $type)
    {
        $config = [
            ...
            'smtp_user' => 'kirimpesan2021@gmail.com',
            'smtp_pass' => 'sidpm2021',
            'smtp_port' => 465,
            'mailtype'  => 'html',
            'charset'   => 'utf-8',
        ...
        $this->email->from('kirimpesan2021@gmail.com', 'SiAspiman');
    ...
    public function verifSipadu()
    {
        $nim = $this->input->post('data1');
        $token_sipadu = $this->input->post('data2');
        // //mengambil post nim
        $this->load->model('Login_model');
        $verifikasi = $this->Login_model->verifikasi_token($nim, $token_sipadu);
        ...
        if ($user) {
            ...
            if ($user['is_active'] == 1) {
                //cek verifikasi
                if ($verifikasi == 1 && $token_sipadu != null) {
                    $data = [
                        'email' => $user['email'],
                        'role_id' => $user['role_id'],
                        'nim' => $user['nim']
                ...
                $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">This account has not been activated!
            </div>');
                redirect('auth');
            }
        } else {
